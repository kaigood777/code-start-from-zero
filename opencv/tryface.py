import cv2
import numpy as np
detector = cv2.CascadeClassifier('D:\\learning\\picture opencv\\face_detect.xml')  # 載入人臉追蹤模型
recog = cv2.face.LBPHFaceRecognizer_create ()      # 啟用訓練人臉模型方法
faces = []   # 儲存人臉位置大小的串列
ids = []     # 記錄該人臉 id 的串列

for i in range(1,21):
    img = cv2.imread('D:\\learning\\picture opencv\\use picture\\{i}.jpg')           # 依序開啟每一張蔡英文的照片
...
for i in range(1,21):
    img = cv2.imread('D:\\learning\\picture opencv\\han picture\\images({i}).jpg')           # 依序開啟每一張川普的照片
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)  # 色彩轉換成黑白
    img_np = np.array(gray,'uint8')               # 轉換成指定編碼的 numpy 陣列
    face = detector.detectMultiScale(gray)        # 擷取人臉區域
    for(x,y,w,h) in face:
